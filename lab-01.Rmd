...
_How many tourist attractions are there in the data set?
...
_How many regions have more than 50 tourist attraction in the data set? (Hint: You will need to tabulate the data before filtering.)97
...
_What are the mean and median visitor numbers in 2022 across all attractions?mean=351942 med=184639.5
...
visitors %>% 
  summarise(
    ...
    mediean_2021=mean(n_2021, na.rm=TRUE), 
    )
```
...
_Which setting (inside, outside or mixed) has the largest mean visitor numbers in 2022? inside
...
_What is the interquartile range (the width of the middle 50% of data set between the lower and upper quartiles) the for each of the four nations of the UK?England	350362.2			
